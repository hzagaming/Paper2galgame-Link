# Paper2Galgame Link

A cinematic, independent link portal designed and curated by **Hanazar Ochikawa**.

🔗 Live website: https://hzagaming.github.io/Paper2galgame-Link/

Current release: **v2.8.4** · [Release announcement](ANNOUNCEMENT.md) · [v2.8.3 archive](docs/announcements/history/v2.8.3.md) · [v2.8.2 archive](docs/announcements/history/v2.8.2.md) · [v2.8.1 archive](docs/announcements/history/v2.8.1.md) · [v2.8.0 archive](docs/announcements/history/v2.8.0.md) · [v2.7.0 archive](docs/announcements/history/v2.7.0.md) · [v2.6.0 archive](docs/announcements/history/v2.6.0.md) · [v2.5.0 archive](docs/announcements/history/v2.5.0.md) · [v2.4.0 archive](docs/announcements/history/v2.4.0.md) · [v2.3.0 archive](docs/announcements/history/v2.3.0.md) · [v2.2.0 archive](docs/announcements/history/v2.2.0.md) · [v2.1.0 archive](docs/announcements/history/v2.1.0.md) · [v2.0.0 archive](docs/announcements/history/v2.0.0.md)

## Features

- Animated, responsive single-page experience with a bounded pointer motion field
- Four curated destinations
- Procedural Web Audio SFX and opt-in ambient BGM
- Accessible, independent SFX and BGM controls
- First-screen destination CTA and touch-safe interactions
- Touch trails, release ripples, and hybrid-pointer depth feedback
- Safe-area-aware layout and high-contrast featured content
- Notch-safe skip navigation and narrow-landscape reflow
- Scroll-momentum choreography, layered card depth, and scene-boundary lighting
- Extreme-text-zoom containment and context-isolated audio recovery
- Reduced-motion support
- Responsive portrait and short-landscape layouts
- Zero runtime dependencies

## Stack

- HTML, CSS, and JavaScript
- GitHub Pages

This is an unofficial portal and does not represent Paper2Galgame.
